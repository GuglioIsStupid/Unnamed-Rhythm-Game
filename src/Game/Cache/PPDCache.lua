local PPDCache = Class:extend("SongCache")

-- What this does is convert the PPDPacks into rit's format
...
function PPDCache:parseToRitc(id, data, script, folderName, packName, diffName, audioname, variations, variationCount)
    local CURRENT_TIME = 0
    local opc, len, params
    local map = {}
    local file = io.open(love.filesystem.getSaveDirectory() .. "/PPDPacks/" .. packName .. "/script/" .. script, "rb")
    if not file then
        return
    end
...
Title: %s
DifficultyName: %s
Description: Automatically generated by PPDCache.lua. A file from Rit by AGORI Studios at %d.
Artist: %s
Source: PPD
Creator: %s
Tags: PPD
...
GameMode: 3
       ...
       -- create .ritc file !!
        ...
        love.filesystem.remove("PPDPacks/" .. packName .. "/sound/song/" .. audioname .. (variations[i] ~= "" and "_" .. variations[i] or "") .. ".ogg")
        love.filesystem.remove("PPDPacks/" .. packName .. "/script/" .. script)
    end
end

function PPDCache:parsePPDPacks(path)
